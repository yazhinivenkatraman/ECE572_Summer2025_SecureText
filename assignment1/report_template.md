...
| 4 | Broken login/logout session flow       | After logout, re-login fails. Client state isn't properly reset.                                    | Prevents legitimate re-login; user must restart client.               | Session Management              | Auth / Availability |
| 5 | List users not functioning             | The `LIST_USERS` feature doesn’t return correct online/all users list.                              | Users can’t discover others, breaking communication flow.             | Least Privilege, Feedback       | Authorization      |
